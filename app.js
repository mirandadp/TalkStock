...
async function syncNow() {
    ...
    try {
        ...
        for (const mv of movs.filter(x => !x.synced)) {
            ...
            const {error} = await SB.from('movimientos').upsert({
                tipo: mv.tipo,
...
async function pullRemoteData() {
    ...
    try {
        ...
        if (rU?.length) {
            ...
            for (const ru of rU) {
                if (!l.find(u => u.remote_id === ru.id || u.id === ru.local_id))
        ...
        if (rM?.length) {
            ...
            for (const rm of rM) {
                ...
                if (ex) {
                    ex.cantidad = rm.cantidad;
                    ex.precio = rm.precio || 0;
                    ex.remote_id = rm.id;
                    ex.synced = 1;
        ...
        if (rP?.length) {
            ...
            for (const rped of rP) {
                const ex = lp.find(p => p.local_id === rped.local_id && rped.local_id);
                if (ex) {
                    ex.estado = rped.estado;
                    ex.synced = 1;
...
async function filterInventory(mats, ubics, ubicMap) {
    ...
    let f = mats;
    if (q)
        f = f.filter(m => norm(m.nombre).includes(q));
    if (fU)
    ...
    if (!el)
        return;
    if (!f.length) {
        el.innerHTML = `<div class="empty"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg><p>${q ? 'Sin resultados' : 'Sin materiales. ¡Empieza añadiendo!'}</p></div>`;
        return;
    }
...
async function updateStats() {
    ...
    if (el('statMat'))
        el('statMat').textContent = mats.length;
    if (el('statUbic'))
        el('statUbic').textContent = ubics.length;
    if (el('statMov'))
        el('statMov').textContent = movs.length;
    if (el('statPend'))
        el('statPend').textContent = movs.filter(m => !m.synced).length;
    updateSyncBadge();
}
...
async function loadSampleData() {
    showConfirmModal('Cargar datos de ejemplo', '<p style="font-size:13px;color:var(--text2);">Se añadirán ubicaciones, materiales y un usuario operario de ejemplo.</p>', async () => {
        ...
        if (!users.find(u => u.nombre === 'Encargado')) {
            await dbAdd('usuarios', {
                nombre: 'Encargado',
                rol: 'encargado',
                pin: '1234',
        ...
        if (!users.find(u => u.nombre === 'Operario 1')) {
            await dbAdd('usuarios', {
                nombre: 'Operario 1',
                rol: 'operario',
                pin: '0000',
...
const PRES = {
    ...
    timersPresentes: [],
    // setInterval por presente (actualizar tiempo)
};
...
function startClock(timeId, dateId) {
    ...
    const tick = () => {
        ...
        if (tEl)
            tEl.textContent = now.toLocaleTimeString('es-ES', {
                hour: '2-digit',
                minute: '2-digit',
                second: '2-digit'
            });
        if (dEl)
            dEl.textContent = now.toLocaleDateString('es-ES', {
                weekday: 'long',
                day: '2-digit',
                month: 'long',
                year: 'numeric'
...
async function onFicharUserChange() {
    ...
    if (!userId) {
        if (dot)
            dot.className = 'estado-dot estado-unknown';
        if (txt)
            txt.textContent = 'Selecciona un operario';
        if (ul)
    ...
    if (dot)
        dot.className = 'estado-dot ' + (dentro ? 'estado-dentro' : 'estado-fuera');
    if (txt) {
        if (dentro) {
            const desde = new Date(ultimo.fecha);
            const dur = calcDur(ultimo.fecha, new Date().toISOString());
            txt.innerHTML = `<strong style="color:var(--success)">${usuario?.nombre}</strong>
         está dentro desde las
         ${desde.toLocaleTimeString('es-ES', {
                hour: '2-digit',
                minute: '2-digit'
            })}
         <span style="color:var(--text3)">(${dur})</span>`;
        } else {
            const lastTxt = ultimo ? ` · Última salida: ${new Date(ultimo.fecha).toLocaleTimeString('es-ES', {
                hour: '2-digit',
                minute: '2-digit'
            })}` : '';
            txt.innerHTML = `<strong style="color:var(--text2)">${usuario?.nombre}</strong>
...
async function renderUltimosFichajesHoy(userId) {
    ...
    const hoy = todos.filter(f => f.userId === userId && f.fecha.startsWith(hoyStr)).sort( (a, b) => new Date(a.fecha) - new Date(b.fecha));
...
async function registrarFichaje(tipo) {
    const userId = parseInt(document.getElementById('fichar-user-sel')?.value);
    if (!userId) {
        toast('Selecciona un operario', 'error');
        return;
    }
    ...
    const usuario = usuarios.find(u => u.id === userId);
    if (!usuario) {
        toast('Operario no encontrado', 'error');
        return;
    }
    ...
    const ultimo = await getUltFichaje(userId);
    if (ultimo?.tipo === tipo) {
        const ok = confirm(`⚠️ ${usuario.nombre} ya tiene una ${tipo} registrada.\n¿Registrar de nuevo?`);
        if (!ok)
            return;
    }
...
async function guardarFichaje({userId, usuario, tipo, nota, creadoPor}) {
    ...
    if (navigator.vibrate)
        navigator.vibrate(tipo === 'entrada' ? [50, 30, 50] : [100]);
...
async function recargarPresencia() {
    ...
    await renderAdminAhora();
    await populateAdminSelectors();
    if (PRES.adminTab === 'historial')
        await renderAdminHistorial();
    if (PRES.adminTab === 'jornadas')
        await renderAdminJornadas();
...
async function renderAdminAhora() {
    ...
    presentes.forEach(f => {
        const update = () => {
            ...
            if (el2)
                el2.textContent = calcDur(f.fecha, new Date().toISOString());
        }
        ;
        update();
...
async function renderAdminHistorial() {
    ...
    let f = [...fichajes];
    if (userFilt)
        f = f.filter(x => x.userId === userFilt);
    if (dateFilt)
        f = f.filter(x => x.fecha.startsWith(dateFilt));
    ...
    if (pi)
        pi.textContent = `Pág. ${PRES.histPage + 1}/${pages} (${total} registros)`;
    const btns = document.querySelectorAll('#fhist-pag button');
    if (btns[0])
        btns[0].disabled = PRES.histPage === 0;
    if (btns[1])
        btns[1].disabled = PRES.histPage >= pages - 1;
    ...
    tbody.innerHTML = page.map(fich => {
        const u = userMap[fich.userId] || {
            nombre: fich.nombreUsuario || '?',
            rol: 'operario'
        };
        const r = ROLES[u.rol] || ROLES.operario;
        const tipo = fich.tipo === 'entrada' ? '<span class="fich-in">↑ ENTRADA</span>' : '<span class="fich-out">↓ SALIDA</span>';
        ...
        let dur = '—';
        if (fich.tipo === 'salida') {
            const entrada = f.find(x => x.userId === fich.userId && x.tipo === 'entrada' && new Date(x.fecha) < fecha);
            if (entrada)
                dur = calcDur(entrada.fecha, fich.fecha);
        }

        const nota = (fich.nota || fich.creadoPor) ? `${fich.nota || ''} ${fich.creadoPor ? '· por ' + fich.creadoPor : ''}`.trim() : '—';
...
async function renderAdminJornadas() {
    ...
    f.forEach(fich => {
        ...
        if (fich.tipo === 'entrada')
            jornadas[key].entradas.push(fich.fecha);
        else
            jornadas[key].salidas.push(fich.fecha);
    }
    );
    ...
    tbody.innerHTML = rows.map(j => {
      ...
      <td style="color:var(--success);">
        ${pE ? new Date(pE).toLocaleTimeString('es-ES', {
            hour: '2-digit',
            minute: '2-digit'
        }) : '—'}
      </td>
      <td style="color:var(--danger);">
        ${uS ? new Date(uS).toLocaleTimeString('es-ES', {
            hour: '2-digit',
            minute: '2-digit'
        }) : '—'}
      </td>
...
function iniciarRTFichajes() {
    ...
    if (PRES.rtChannel) {
        try {
            SB.removeChannel(PRES.rtChannel);
        } catch (e) {}
        PRES.rtChannel = null;
    }

    PRES.rtChannel = SB.channel('pres-admin-live').on('postgres_changes', {
        event: 'INSERT',
        schema: 'public',
        table: 'fichajes'
    }, async payload => {
        ...
        toast(`${tipo} ${r.nombre_usuario || ''} — ${new Date(r.fecha).toLocaleTimeString('es-ES', {
            hour: '2-digit',
            minute: '2-digit'
        })}`, '');
    }
...
function setRTStatus(live, label) {
    ...
    if (dot)
        dot.className = 'rt-dot' + (live ? ' live' : '');
    if (lbl)
        lbl.textContent = label || '';
}

// ══════════════════════════════════════════════════════════
// SINCRONIZACIÓN SUPABASE (subir fichajes locales)
// Añadir en syncNow() del index.html:
/*
  const fichs = await dbGetAll('fichajes');
  for (const f of fichs.filter(x => !x.sinc)) {
    const { error } = await SB.from('fichajes').upsert({
...
*/
// Y en pullRemoteData() añadir:
/*
  const { data: rFichs } = await SB.from('fichajes').select('*').gt('created_at', lastPull);
  if (rFichs?.length) {
    const local = await dbGetAll('fichajes');
    for (const r of rFichs) {
      if (!local.find(f => f.local_id === r.local_id && r.local_id)) {
        await dbAdd('fichajes', {
          userId: r.user_id, nombreUsuario: r.nombre_usuario,
          rolUsuario: r.rol_usuario, tipo: r.tipo, fecha: r.fecha,
          fechaLocal: r.fecha_local, creadoPor: r.creado_por,
          dispositivo: r.dispositivo, nota: r.nota || '',
          local_id: r.local_id, sinc: 1
        });
      }
    }
  }
...
async function exportFichajesCSV() {
    ...
    csv += fichajes.map(f => {
        ...
        return [f.id, `"${u.nombre}"`, u.rol, f.tipo, f.fecha, `"${f.fechaLocal || ''}"`, `"${f.creadoPor || ''}"`, `"${f.nota || ''}"`, `"${(f.dispositivo || '').substring(0, 40)}"`].join(',');
    }
    ).join('\n');
...
async function exportResumenCSV() {
    ...
    [...fichajes].sort( (a, b) => new Date(a.fecha) - new Date(b.fecha)).forEach(f => {
        ...
        if (f.tipo === 'entrada')
            jornadas[key].entradas.push(f.fecha);
        else
            jornadas[key].salidas.push(f.fecha);
    }
    );
    ...
    Object.values(jornadas).sort( (a, b) => a.dia.localeCompare(b.dia)).forEach(j => {
        ...
        csv += [`"${u.nombre}"`, u.rol, j.dia, pE ? new Date(pE).toLocaleTimeString('es-ES', {
            hour: '2-digit',
            minute: '2-digit'
        }) : '—', uS ? new Date(uS).toLocaleTimeString('es-ES', {
            hour: '2-digit',
...
async function getUltFichaje(userId) {
    const todos = await dbGetAll('fichajes');
    return [...todos].filter(f => f.userId === userId).sort( (a, b) => new Date(b.fecha) - new Date(a.fecha))[0] || null;
}
